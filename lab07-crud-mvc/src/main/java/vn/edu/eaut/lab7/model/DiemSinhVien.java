...
public class DiemSinhVien {
    ...
    public DiemSinhVien(int id, String maSinhVien, String hoTen,
    ...
    public double getDiemTongKet() {
        return Math.round((diemChuyenCan * 0.1 + diemGiuaKy * 0.3 + diemCuoiKy * 0.6) * 100.0) / 100.0;
    }

    public String getXepLoai() {
        double tk = getDiemTongKet();
        if (tk >= 8.5) return "A";
        if (tk >= 7.0) return "B";
        if (tk >= 5.5) return "C";
        if (tk >= 4.0) return "D";
